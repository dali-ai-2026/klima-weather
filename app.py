...
with streamlit_analytics.track():
    ...
    with col1:
        ...
        if china_news:
            first = china_news[0]
            st.markdown(f'<a href="{first["url"]}" target="_blank" rel="noopener noreferrer" style="text-decoration: none; font-weight: bold;">📰 {first["title"][:60]} ➤</a>', unsafe_allow_html=True)
                ...
                with st.expander(f"📖 もっと見る（{len(china_news)-1}件）"):
                    for news in china_news[1:]:
                        if news.get('title', '').strip():
                            st.markdown(f'<a href="{news["url"]}" target="_blank" rel="noopener noreferrer" style="text-decoration: none;">• {news["title"][:50]} ➤</a>', unsafe_allow_html=True)
        else:
            st.info("暂无国内气象新闻")
